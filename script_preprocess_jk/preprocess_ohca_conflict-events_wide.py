"""Preprocess conflict events (OHCA) - Wide.
The task is to predict fatalities. (regression)
guide: 
"""
...
filename = 'temp_ohca_confilicts/hdx_hapi_conflict_event_global_2024.csv' # Change the filename accordingly
...
# if we keep data at admin level 0 - we are not left with much textual data.
data = data[data['admin_level'] != '0'].reset_index(drop=True)
data['events'] = data['events'].astype('float32')
data['fatalities'] = data['fatalities'].astype('float32')
groupby_col = [
    'location_code', 'has_hrp', 'in_gho', 'provider_admin1_name', 'provider_admin2_name',
    'admin1_code', 'admin1_name', 'admin2_code', 'admin2_name', 
    'event_type', 'dataset_hdx_id', 'resource_hdx_id', 'warning',
]
data = data.groupby(groupby_col, as_index=False).agg({
    'events': 'sum',
    'fatalities': 'sum'
})
...
data_filename = 'conflict-events_wide'
