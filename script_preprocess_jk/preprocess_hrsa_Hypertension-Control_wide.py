"""Preprocess Hypertension Control. - wide
The goal is to achieve 61.2.
target: Estimated Percentage of Patients with Controlled Blood Pressure
...
filename = 'health_resources_and_service_administration/FS_EHB_AWARD_GRANT_FA_AGR_MVX.xlsx'
data_path = f'{path_configs['base_path']}/data/data_raw/{filename}'
data_grant = pd.read_excel(data_path, sheet_name='FS_EHB_AWARD_GRANT_FA_AGR_MVX')
filename = 'health_resources_and_service_administration/Hypertension NHCI UDS 2019-2023.xlsx'
data_path = f"{path_configs['base_path']}/data/data_raw/{filename}"
data = pd.read_excel(data_path, sheet_name='2023')

# Process for grant information
data_grant = data_grant[data_grant['Award Year'] == 2023].reset_index(drop=True)
grant_number = data['Grant Number'].tolist()
data_grant = data_grant[data_grant['Grant Number'].isin(grant_number)]
data_grant = data_grant.drop_duplicates(subset='Grant Number').reset_index(drop=True)
data = data.merge(how='left', right=data_grant, on='Grant Number')
...
## Set metadata
target_name = 'Estimated Percentage of Patients with Controlled Blood Pressure'
...
# Target engineering
target_mask = data[target_name].copy()
data.loc[target_mask >= 0.612, target_name] = 1
data.loc[target_mask < 0.612, target_name] = 0
...
data_filename = 'hypertension-control-wide'
