"""Preprocess ohca-fts-funding.
Aggregates three different funding types.
The task is to predict amountUSD (regression).
"""
...
filenames = []
filenames.append('temp_ohca_funding/fts_outgoing_funding_global.csv')
filenames.append('temp_ohca_funding/fts_incoming_funding_global.csv')
filenames.append('temp_ohca_funding/fts_internal_funding_global.csv')

data = pd.DataFrame()
for filename in filenames:
    data_path = f'{path_configs['base_path']}/data/data_raw/{filename}'
    data_ = pd.read_csv(data_path)
    data_['funding_type'] = data_path.split('/')[-1].split('.csv')[0].replace('fts_', '').replace('_global', '')
...
drop_col = []
# # Leakage
drop_col.append('originalAmount')
# # # High correlation
drop_col.append('onBoundary')
drop_col.append('flowType')
