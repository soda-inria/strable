"""Preprocess fueleconomy-tax-incentives.
The task is to predict the tax incentives of vehicle models. (regression)
refer to https://www.fueleconomy.gov/feg/ws/tax-data-services.shtml
"""
...
filepath = glob(f'{path_configs['base_path']}/data/data_raw/automobile/*public*')
data = pd.DataFrame()
for file in filepath:
    with open(file) as xml_file:
        data_dict = xmltodict.parse(xml_file.read())

    marker1 = list(data_dict.keys())[0]
    marker2 = marker1[:-1]
    marker3 = list(data_dict[marker1][marker2][0].keys())[-1]
    marker4 = marker3[:-4]

    df = pd.DataFrame()
    for item in data_dict[marker1][marker2]:
        data_ = item[marker3][marker4]
        if not isinstance(data_, list):
            data_ = [data_]
        data_ = pd.DataFrame(data_)
        manu_key = [key for key in item.keys() if 'anufacturer' in key]
        data_['manufacturer'] = item[manu_key[0]]
...
data.fullcreditamount = data.fullcreditamount.astype(float)
data.creditamount = data.creditamount.astype(float)
data.taxcredittotal = data.taxcredittotal.astype(float)
