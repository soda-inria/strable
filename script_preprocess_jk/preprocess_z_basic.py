"""Preprocess worldbankfinancesone-financial-intermediary-funds-funding-decisions.
The task is to predict Amount in USD (regression)
"""
...
folder = '/data/parietal/store3/work/mkim/codes/salts/data/data_processed/'
len(glob(f'{folder}*'))
...
filename = 'worldbankfinancesone/financial_intermediary_funds_funding_decisions_07-12-2025.csv'
data_path = f'{path_configs['base_path']}/data/data_raw/{filename}'

data_path = '/data/parietal/store3/work/mkim/codes/salts/data/data_raw/doe_eligibility/fy2025_master_eligibility_spreadsheet-forpublic.xlsx'
data = pd.read_excel(data_path, sheet_name='Eligibility-All LEAs')
...
## Set metadata
target_name = 'Amount in USD'
task = 'regression'
task_type = 'wide'
source = 'worldbankfinancesone'

## Clean for the target column
data = data[data[target_name]>0].reset_index(drop=True)
...
data_filename = 'financial-intermediary-funds-funding-decisions'
