...
def embed_table(data_name, n_split, fold_index, embed_method, normalization, no_pca, n_dimensions):
    ...
    # Load data
    data_folder = f"{path_configs['path_data_processed']}_feature_eng"
...
def prepare_llm(X_train, X_test, data_name, llm_model_name, normalization, n_dimensions=30):
    ...
    # Encode
    cached_llm_embedding_path = f"{path_configs['base_path']}/data/llm_embeding_feat_eng/{llm_model_name}/{llm_model_name}|{data_name}.parquet"
    ...
    # Extraction time
    time_folder = f"{path_configs['base_path']}/data/llm_embed_time_feat_eng/{llm_model_name}"
