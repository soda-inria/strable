...
def embed_table(data_name, n_split, fold_index, embed_method, normalization, no_pca, n_dimensions, missing_val_policy="none"):
    ...
    X_train, X_test, y_train, y_test = apply_missing_value_policy(
        X_train, X_test, y_train, y_test, missing_val_policy, cat_col_names
    )
    ...
    elif encode_method == "mambular":
        X_train, X_test = prepare_mambular(X_train, X_test)
...
def apply_missing_value_policy(X_train, X_test, y_train, y_test, policy, cat_col_names=None):
    """Apply a missing value handling policy after train/test split."""
    if policy == "none":
        return X_train, X_test, y_train, y_test

    if policy == "drop_all":
        # Drop rows with any NaN from train
        train_mask = ~X_train.isnull().any(axis=1)
        X_train = X_train[train_mask].reset_index(drop=True)
        y_train = y_train[train_mask] if isinstance(y_train, pd.Series) else y_train[train_mask.values]
        
        # Drop rows with any NaN from test
        test_mask = ~X_test.isnull().any(axis=1)
        X_test = X_test[test_mask].reset_index(drop=True)
        y_test = y_test[test_mask] if isinstance(y_test, pd.Series) else y_test[test_mask.values]
        ...
        print(f"  drop_all: train {train_mask.sum()}/{len(train_mask)} rows kept, "
              f"test {test_mask.sum()}/{len(test_mask)} rows kept")
        return X_train, X_test, y_train, y_test

    if policy == "uniform_impute":
        ...
        print(f"  uniform_impute: {len(num_cols)} num (mean), {len(cat_cols)} cat (mode)")
        return X_train, X_test, y_train, y_test

    raise ValueError(f"Unknown missing_val_policy: {policy}")
...
def prepare_mambular(X_train, X_test):
    """Function to prepare with mambular"""
