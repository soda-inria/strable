"""Preprocess medicaid national average drug acquisition cost dataset.
The task is to predict NADAC Per Unit (regression).
"""
...
# Reduced for faster load
# filename = 'medicaid/nadac-national-average-drug-acquisition-cost-2024_reduced.csv'
# data = data.sort_values(by='As of Date').reset_index(drop=True)
# data = data.drop_duplicates(subset=['NDC Description', 'NDC'], keep='last').reset_index(drop=True)
# data = data.drop_duplicates(subset=['NDC Description', 'NADAC Per Unit'], keep='last').reset_index(drop=True)
...
data_filename = 'national-average-drug-acquisition-cost'
