"""Preprocess european-commission_rasnf.
task: predict of Number of countries effected (regression)
...
filename = 'european-commission_rasnf/RASNF_window.csv'
...
# create a new target column 'num_countries_affected' based on 'Products were found and measures were taken also in'
# if 'Products were found and measures were taken also in' is NaN, set 'num_countries_affected' to 0 else count the number of countries separated by commas
data['num_countries_affected'] = data['Products were found and measures were taken also in'].apply(
    lambda x: 1 if pd.isna(x) else len(str(x).split(','))+1
)
...
## Set metadata
target_name = 'num_countries_affected'
...
data.loc[temp == '1', target_name] = 'One country'
data.loc[temp != '1', target_name] = 'More than one country'
...
data_filename = 'rasnf_notification_list'
