"""Preprocess flavors-of-cacao_chocolate-bar-ratings."""
...
filename = 'carte/flavors-of-cacao_chocolate-bar-ratings.csv' # Change the filename accordingly
...
data.groupby(['Review Date','Company (Manufacturer)', 'Country of Bean Origin',
       'Specific Bean Origin or Bar Name', 'Cocoa Percent'])['Rating'].nunique().drop_duplicates()

# can we follow the same partition along the years?
data.groupby(['Company (Manufacturer)', 'Country of Bean Origin',
       'Specific Bean Origin or Bar Name', 'Cocoa Percent'])['Review Date'].nunique().value_counts()

# there are only 3 combinations that have more than 1 review date - this is wide
data.groupby(['Company (Manufacturer)', 'Country of Bean Origin',
...
target_pseudo = data[target_name].copy()
data[target_name] = data[target_name].astype(str)
data.loc[target_pseudo>=3.75, target_name] = 'Over 3.75'
data.loc[target_pseudo==3.5, target_name] = '3.5'
data.loc[target_pseudo==3.25, target_name] = '3.25'
data.loc[target_pseudo==3, target_name] = '3'
data.loc[target_pseudo<=2.75, target_name] = 'Under 2.75'
...
data_filename = 'chocolate-bar-ratings'
