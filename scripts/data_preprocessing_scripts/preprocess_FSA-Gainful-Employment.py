"""Preprocess FSA Gainful Employment.
The task is to predict Debt-to-Earnings Annual Rate (regression)
"""
...
filename = 'fsa_height_cash/GE-DMYR-2015-Final-Rates.xls' # Modified for faster loading
data_path = f'{path_configs['base_path']}/data/data_raw/{filename}'
data = pd.read_excel(data_path, sheet_name='GE Rates Final DMYR 2015')

## Clean for backslash operations
data = clean_backslash_operations(data)

## Dataset-level specific cleaning
debt_col = [col for col in data.columns if 'Debt' in col]
for col in debt_col:
    if 'Pass/Fail/Zone' in col:
        debt_col.remove(col)
for col in debt_col:
...
## Set metadata
target_name = 'Debt-to-Earnings Annual Rate'
task = 'regression'
task_type = 'wide'
source = 'FSA'
...
drop_col = []
# Leakage
drop_col.append('Debt-to-Earnings Annual Rate Pass/Fail/Zone') 
drop_col.append('Debt-to-Earnings Annual Rate Numerator') 
drop_col.append('Debt-to-Earnings Annual Rate Denominator')
drop_col.append('Mean Annual Earnings From SSA')
drop_col.append('Median Annual Earnings from SSA')
drop_col.append('Official Program Pass/Zone/Fail')
# Highly correlated
drop_col.append('Debt-to-Earnings Transitional Rate')
drop_col.append('Debt-to-Earnings Transitional Rate Numerator')
drop_col.append('Debt-to-Earnings Transitional Rate Denominator')
drop_col.append('Debt-to-Earnings Transitional Discretionary Income Rate')
drop_col.append('Debt-to-Earnings Transitional Discretionary Income Rate Numerator')
drop_col.append('Debt-to-Earnings Transitional Discretionary Income Rate Denominator')
drop_col.append('Debt-to-Earnings Discretionary Income Rate')
drop_col.append('Debt-to-Earnings Discretionary Income Rate Numerator')
drop_col.append('Debt-to-Earnings Discretionary Income Rate Denominator')
# Unknown before the target
drop_col.append('Official Appeal Decision')
...
data_filename = 'gainful-employment'
