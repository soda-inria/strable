"""Preprocess fueleconomy-vehicles.
The task is to predict the amount to save/spend over 5 years compared to an average car. (regression)
refer to https://www.fueleconomy.gov/feg/ws/index.shtml#vehicle
"""
...
filename = 'automobile/fuel-economy_vehicles.csv' # Change the filename accordingly
...
# We are interested in the year 2025
# The target relates over several years; to prevent leakage
...
# Information regarding costs (possible leakage)
drop_col = ['UCity',
 'UCityA',
 'UHighway',
 'UHighwayA',
...
drop_col.append('range')
drop_col.append('rangeHwy')
drop_col.append('rangeCity')
drop_col.append('co2TailpipeGpm')
drop_col.append('co2')
drop_col.append('displ')
drop_col.append('cylinders')
...
# # Check with Cramer's-V
# temp = pd.concat([X_train, pd.Series(y_train)], axis=1)
# cram_df = column_associations(temp)
# cram_df1 = cram_df[cram_df.left_column_name=='0'].copy()
# cram_df2 = cram_df[cram_df.right_column_name=='0'].copy()
# cram_df = pd.concat([cram_df1, cram_df2])
# cram_df = cram_df.sort_values(by='cramer_v', ascending=False).reset_index(drop=True)
# cram_df.iloc[:10]
