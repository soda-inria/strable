"""Preprocess aijob_ai-ml-ds-salaries."""
...
filename = 'carte/aijob_ai-ml-ds-salaries.csv' # Change the filename accordingly
...
data.groupby(['work_year', 'experience_level', 'employment_type', 'job_title',
        'employee_residence',
       'remote_ratio', 'company_location', 'company_size'])['salary'].nunique()

# noticed we have twice the salary for the same partition, so i aggregate
data = data.groupby(['work_year', 'experience_level', 'employment_type', 'job_title', 'salary_currency',
        'employee_residence', 'remote_ratio', 'company_location', 'company_size'], as_index=False).agg({'salary':'mean', 'salary_in_usd':'mean'})

data.groupby(['work_year', 'experience_level', 'employment_type', 'job_title', 'salary_currency',
        'employee_residence', 'remote_ratio', 'company_location', 'company_size'])['salary'].nunique().value_counts()

# majority have one work_year --> it's a pyramid structure (wide) not block (block=panel)
...
data_filename = 'aijob_ai-ml-ds-salaries'
...
data_name = 'aijob_ai-ml-ds-salaries'
n_split = 3
fold_index = 1
