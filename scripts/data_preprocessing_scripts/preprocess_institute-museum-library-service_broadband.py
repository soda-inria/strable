"""Preprocess institute-museum-library-service_broadband.
The task is to predict the population for whom broadband is available. (regression)
refer to https://www.imls.gov/grants/awarded-grants
"""
...
filename = 'imls/publicdata_imls_metrics.xlsx'
data_path = f'{path_configs['base_path']}/data/data_raw/{filename}'
data = pd.read_excel(data_path, sheet_name='County Data REV')
...
## Set metadata
target_name = 'Population for whom broadband available, 2019 (%)'
task = 'regression'
task_type = 'wide'
...
data_filename = 'broadband-availability'
