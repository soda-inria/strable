"""Preprocess World Resource Institute - Global Power Plant.
The task is to predict the capacity of the power plants. (regression)
refer to 
"""
...
filename = 'world_resource_institute/global_power_plant_database.csv' # Change the filename accordingly
...
drop_col = []
drop_col += [col for col in data.columns if 'generation_gwh' in col]
drop_col += [col for col in data.columns if 'estimated' in col]
...
data_filename = 'global-power-plant'
