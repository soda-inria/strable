"""Preprocess device-covid19serology."""
...
filename = 'fda/device-covid19serology.json' # Change the filename accordingly
...
drop_col = []
# These predictors have high cramer's-v
titer_col = [col for col in data.columns if 'titer' in col]
agree_col = [col for col in data.columns if 'agree' in col]
truth_col = [col for col in data.columns if 'truth' in col]
truth_col.remove('antibody_truth')
drop_col = titer_col+agree_col+truth_col
drop_col.append('type')
# igg, igm and related information are leakages
drop_col.append('group') # leakage
drop_col.append('days_from_symptom') # leakage
drop_col.append('igg_result') # leakage
drop_col.append('igm_igg_result') # leakage
drop_col.append('igm_result') # leakage
drop_col.append('igm_iga_result') # leakage
drop_col.append('sample_id') # leakage
...
data_filename = 'device-covid19serology'
