"""Preprocess medicaid child adult healthcare quality.
The task is to predict the State Rate (regression).
"""
...
filename = 'medicaid/child-and-adult-health-care-quality-measures.csv'
data_path = f"{path_configs['base_path']}/data/data_raw/{filename}"
data = pd.read_csv(data_path, low_memory=False)

mask = data['Core Set Year'].isnull()
data.loc[mask, 'Core Set Year'] = data.loc[mask, 'FFY']
data.drop(columns='FFY', inplace=True)

mask = data['State Specific Comments'].isnull()
data.loc[mask, 'State Specific Comments'] = data.loc[mask, 'State-Specific Comments']
data.drop(columns='State-Specific Comments', inplace=True)

data = data[data['Core Set Year']>2018].reset_index(drop=True)
data.drop(columns='Mean', inplace=True)

data = data.sort_values(by='Core Set Year').reset_index(drop=True)
...
drop_col = []
drop_col.append('Median')
drop_col.append('Top Quartile')
drop_col.append('Bottom Quartile')
...
data_filename = 'child-adult-healthcare-quality'
