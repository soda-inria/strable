"""Main evaluation pipeline: modular ``{encoder}`` -> 30-PCA -> tabular learner.
"""
...
def run_model(
    ...
    # Ablation folder is derived from --no_pca: results/no-pca/... when set,
    # results/default/... otherwise. The dropped script_evaluate_nopca.py had
    # the same behavior (ABLATION = "no-pca" hard-coded).
    ablation = "no-pca" if no_pca else "default"
    save_path = f"./results/{ablation}/{save_dir}"
    flag_suffix = ""
    if normalization:
        flag_suffix += "_norm"
    if n_dimensions != 30:
        flag_suffix += f"_pc{n_dimensions}"
    method_marker = f"{method}_{tune_indicator}{flag_suffix}"
    ...
    elif estim_method in ["tabstar", "tabicl", "mambular"]: # learners that do not want an array but want a Pandas DataFrame
        if estim_method == "mambular":
            if isinstance(X_train, pd.DataFrame):
                # Drop constant columns
                const_cols = [c for c in X_train.columns if X_train[c].nunique() <= 1]
                if const_cols:
                    print(f"  Dropping {len(const_cols)} constant columns for mambular")
                    X_train = X_train.drop(columns=const_cols)
                    X_test = X_test.drop(columns=const_cols)
                # Sanitize column names: ColumnTransformer forbids '__'
                rename_map = {c: c.replace("__", "_") for c in X_train.columns if "__" in c}
                if rename_map:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Smoke-test mode: limit to the first dataset and fold 0 only.",
    )
    ...
    if args.dry_run:
        # Take the first dataset / fold combination only.
        args_list = [args_list[0]] if args_list else args_list
        args_list[0]["fold_index"] = 0
        print(f"[dry-run] Limited to one experiment: {args_list[0]}")
