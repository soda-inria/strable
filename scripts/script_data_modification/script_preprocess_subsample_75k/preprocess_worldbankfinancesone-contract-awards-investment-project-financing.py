"""Preprocess worldbankfinancesone-contract-awards-investment-project-financing.
The task is to predict the supplier contract amount
"""
...
# filename = 'worldbankfinancesone/contract_awards_in_investment_project_financing.csv'
# data_path = f'{path_configs['base_path']}/data/data_raw/{filename}'
data_path = f"{path_configs['path_data_raw']}/worldbankfinancesone/contract_awards_in_investment_project_financing_since_fy_2020_03-26-2026.csv"
data = pd.read_csv(data_path, low_memory=False)

#check with existing dataset format
existing_data_path = f"{path_configs['path_data_processed']}/contract-awards-investment-project-financing/data.parquet"
existing_data = pd.read_parquet(existing_data_path)

#common columns
common_cols = set(data.columns).intersection(set(existing_data.columns))
print(f"Common columns: {common_cols}")
print(f"Number of common columns: {len(common_cols)/len(existing_data.columns)*100:.2f}%")

#different columns
diff_cols_new = set(data.columns) - set(existing_data.columns)
diff_cols_existing = set(existing_data.columns) - set(data.columns)
...
# col2 = 'Supplier Contract Amount (USD)'
# data = data.dropna(subset=[col2], how='all').reset_index(drop=True)
...
## Set metadata
target_name = 'Supplier Contract Amount (USD)'
task = 'regression'
task_type = 'wide'
...
data[target_name] = np.arcsinh(data[target_name]) # applied
...
#         data, _ = train_test_split(data, train_size=75000, random_state=42, stratify=data['Fiscal Year'])
#     else:
...
if set(data.columns) != set(existing_data.columns):
    print("Columns in new dataset but not in existing dataset:")
    print(set(data.columns) - set(existing_data.columns))
    print("Columns in existing dataset but not in new dataset:")
    print(set(existing_data.columns) - set(data.columns))

## Save data and config
## Change appropriately the save_folder
data_filename = 'contract-awards-investment-project-financing-FULL'
