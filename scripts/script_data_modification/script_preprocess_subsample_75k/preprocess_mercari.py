"""
Preprocess mercari (from textTabBench)
The task is to predict price (regression)
"""
...
# filename = 'textTabBench/mercari_train.tsv' # Change the filename accordingly
# data_path = f'{path_configs['base_path']}/data/data_raw/{filename}'
data_path = f"{path_configs['ttb_datasets']}/paper_datasets/regression/mercari_price_data.tsv"
data = pd.read_csv(data_path, sep='\t')

# check dataset format matches existing one
mercari_df = pd.read_parquet(f"{path_configs['path_data_processed']}/mercari/data.parquet")

#common columns
common_cols = set(data.columns).intersection(set(mercari_df.columns))
print(f"Common columns: {common_cols}")
print(f"Number of common columns: {len(common_cols)/len(mercari_df.columns)*100:.2f}%")
