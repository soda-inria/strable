...
# filename = 'fda/cosmetic-event.json' # Change the filename accordingly
# data_path = f'{path_configs['base_path']}/data/data_raw/{filename}'
data_path = f"{path_configs['path_data_raw']}/FDA/cosmetic-event.json"
...
cosmetic_df = pd.read_parquet(f"{path_configs['path_data_processed']}/cosmetic-event/data.parquet")

#common columns
common_cols = set(data.columns).intersection(set(cosmetic_df.columns))
print(f"Common columns: {common_cols}")
print(f"Number of common columns: {len(common_cols)/len(cosmetic_df.columns)*100:.2f}%")

#different columns
diff_cols_new = set(data.columns) - set(cosmetic_df.columns)
diff_cols_existing = set(cosmetic_df.columns) - set(data.columns)
...
data = clean_dict_type_col(data, col='patient', clean_type='unpack')
data = clean_dict_type_col(data, col='products', clean_type='to_string')
# Clean for list-type columns
list_type_col = []
list_type_col.append('outcomes')
list_type_col.append('reactions')
list_type_col.append('products')
for col in list_type_col:
...
# Specific target engineering
temp = data[target_name].copy()
temp[temp.str.contains('Death')] = 'Death'
temp[temp.str.contains('Hospitalization')] = 'Hospitalization'
temp[temp.str.contains('Other Serious or Important Medical Event')] = 'Other Serious or Important Medical Event'
temp = temp.str.split(',').str[0]
data[target_name] = temp
class_names = ['Death', 'Other Serious or Important Medical Event', 'Hospitalization']
data = data[data[target_name].isin(class_names)]
data.reset_index(drop=True, inplace=True)
# To prevent leakages
temp = data['reactions'].copy()
temp = temp.str.replace('Death, ', '')
temp = temp.replace('Death', np.nan)
data['reactions'] = temp
...
if set(data.columns) != set(cosmetic_df.columns):
    print("Columns in new dataset but not in existing dataset:")
    print(set(data.columns) - set(cosmetic_df.columns))
    print("Columns in existing dataset but not in new dataset:")
    print(set(cosmetic_df.columns) - set(data.columns))


## Save data and config
## Change appropriately the save_folder
data_filename = 'cosmetic-event-FULL'
