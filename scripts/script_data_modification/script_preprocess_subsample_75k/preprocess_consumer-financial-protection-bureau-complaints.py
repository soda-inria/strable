"""Preprocess Consumer-Financial-Protection-Bureau-Financial-Product-Complaint
The task is to predict Company response to consumer (classification)
"""
...
# filename = 'Consumer Financial Protection Bureau/complaints_reduced_2025.csv'
# data_path = f'{path_configs['base_path']}/data/data_raw/{filename}'

# The raw `complaints.csv` is ~8GB; the version used here is a pre-reduced
# extract (`complaints_reduced_2025.csv`).
data_path = f"{path_configs['path_data_raw']}/Consumer_Financial_Protection_Bureau/complaints_reduced_2025.csv"
# data = pd.read_csv(data_path)
data = pd.read_csv(data_path, on_bad_lines='skip', engine='c')

data.head()

# check dataset format matches existing one
complaint_df = pd.read_parquet(f"{path_configs['path_data_processed']}/financial-product-complaint/data.parquet")


# check common columns with intersection and difference
common_cols = set(data.columns).intersection(set(complaint_df.columns))
print(f"Common columns: {common_cols}")
print(f"Number of common columns: {len(common_cols)/len(complaint_df.columns)*100:.2f}%")
diff_cols_new = set(data.columns) - set(complaint_df.columns)
diff_cols_existing = set(complaint_df.columns) - set(data.columns)
print(f"Columns in the new dataset but not in the existing dataset: {diff_cols_new}")
print(f"Columns in the existing dataset but not in the new dataset: {diff_cols_existing}")

# drop 'Consumer disputed?', 'Timely response?' as they are not in the existing dataset
data.drop(columns=['Consumer disputed?', 'Timely response?'], inplace=True)

## Clean for backslash operations
data = clean_backslash_operations(data)

## Dataset-level specific cleaning
#take only 2025 year as no compaint id's are unique and not followed over years
# temp = data[['Date received']].copy()
# tabvec = TableVectorizer()
# temp = tabvec.fit_transform(temp)
# temp_year = temp['Date received_year'].copy()
# data = data[temp_year == 2025].reset_index(drop=True)
...
## Set metadata
target_name = 'Company response to consumer'
...
# drop_col = []
# drop_col.append('Timely response?') # Leakage
