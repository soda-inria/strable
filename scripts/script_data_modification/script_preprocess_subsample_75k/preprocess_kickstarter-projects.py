...
# filename = 'textTabBench/Kickstarter_projects_Feb19.csv' # Change the filename accordingly
# data_path = f'{path_configs['base_path']}/data/data_raw/{filename}'
data_path = f"{path_configs['ttb_datasets']}/paper_datasets/classification/kickstarter_data.csv"
data = pd.read_csv(data_path)

#check with existing dataset
kick_df = pd.read_parquet(f"{path_configs['path_data_processed']}/kickstarter-projects/data.parquet")

#common columns
common_cols = set(data.columns).intersection(set(kick_df.columns))
print(f"Common columns: {common_cols}")
print(f"Number of common columns: {len(common_cols)/len(kick_df.columns)*100:.2f}%")
