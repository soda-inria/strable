"""Preprocess FSA Foreign Gift and Contract.
The task is to predict amount (regression).
"""
...
# filename = 'fsa_foreign_contract/Sec117PublicRecordsCompleteFeb2025.xlsx' # Modified for faster loading
# data_path = f'{path_configs['base_path']}/data/data_raw/{filename}'
data_path = f"{path_configs['path_data_raw']}/FSA/Sec117PublicRecordsCompleteFeb2025.xlsx"
...
# check with existing dataset
foreign_gift_df = pd.read_parquet(f"{path_configs['path_data_processed']}/foreign-gift-and-contract/data.parquet")

#check common and different columns
common_cols = set(data.columns).intersection(set(foreign_gift_df.columns))
print(f"Common columns: {common_cols}")
print(f"Number of common columns: {len(common_cols)/len(foreign_gift_df.columns)*100:.2f}%")
...
if set(data.columns) != set(foreign_gift_df.columns):
    print("Columns in new dataset but not in existing dataset:")
    print(set(data.columns) - set(foreign_gift_df.columns))
    print("Columns in existing dataset but not in new dataset:")
    print(set(foreign_gift_df.columns) - set(data.columns))

## Save data and config
## Change appropriately the save_folder
data_filename = 'foreign-gift-and-contract-FULL'
