"""Preprocess worldbankfinancesone-disbursements-in-trust-funds.
The task is to predict Disbursements USD (Trust funds) (regression)
"""
...
# filename = 'worldbankfinancesone/disbursements_in_trust_funds_07-13-2025.csv'
# data_path = f'{path_configs['base_path']}/data/data_raw/{filename}'
data_path = f"{path_configs['path_data_raw']}/worldbankfinancesone/disbursements_in_trust_funds_03-26-2026.csv"
data = pd.read_csv(data_path, low_memory=False)

#check with existing dataset format
existing_data_path = f"{path_configs['path_data_processed']}/disbursements-in-trust-funds/data.parquet"
...
## Set metadata
target_name = 'Cash Disbursements (US$)'
task = 'regression'
task_type = 'wide'
...
# Target engineering (per million)
data[target_name] = data[target_name] / 1000000
...
if task == 'regression':
    ...
    if abs(skewness) > 1:
        check_y = data[target_name].copy()
        check_y = np.sign(check_y) * np.log1p(np.abs(check_y))
        skewness = stats.skew(check_y)
        kurtosis = stats.kurtosis(check_y)
...
data[target_name] = np.sign(data[target_name]) * np.log1p(np.abs(data[target_name])) # applied
...
data_filename = 'disbursements-in-trust-funds-FULL'
