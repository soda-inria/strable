"""Preprocess yelp_business dataset."""
...
from src.utils_preprocess import clean_backslash_operations, clean_constant_null, clean_dict_type_col
from configs.path_configs import path_configs

## Load data (This may be different for specific cases)
# filename = 'carte/yelp_academic_dataset_business.json' # Change the filename accordingly
# data_path = f'{path_configs['base_path']}/data/data_raw/{filename}'
data_path = f"{path_configs['path_data_raw']}/carte/data/single_tables/yelp_academic_dataset_business.json"
...
existing_data_path = f"{path_configs['path_data_processed']}/yelp_business/data.parquet"
...
data = clean_dict_type_col(data, 'attributes', 'to_string')
data = clean_dict_type_col(data, 'attributes', 'to_string')
data = clean_dict_type_col(data, 'hours', 'to_string')
...
data_filename = 'yelp_business-FULL'
