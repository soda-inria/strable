"""
Preprocess HIFLD Transmission Lines dataset
The task is to predict VOLTAGE (regression)
"""
...
# filename = 'HIFLD/Transmission_Lines.csv'
# data_path = f'{path_configs['base_path']}/data/data_raw/{filename}'
data_path = f"{path_configs['path_data_raw']}/HIFLD/US_Electric_Power_Transmission_Lines_-3542134192296472621.csv"
data = pd.read_csv(data_path)

transmission_map = {
    'Object ID (Internal)': 'OBJECTID',
    'Transmission Line ID': 'ID',
    'Line Type': 'TYPE',
    'Operational Status': 'STATUS',
    'Source Date': 'SOURCEDATE',
    'Validation Method': 'VAL_METHOD',
    'Validation Date': 'VAL_DATE',
    'Transmission Line Owner': 'OWNER',
    'Voltage (Kilovolts)': 'VOLTAGE',
    'Voltage Class': 'VOLT_CLASS',
    'Inferred Attributes': 'INFERRED',
    'Shape Length': 'Shape__Length'
}

# 1. Rename the columns
data.rename(columns=transmission_map, inplace=True)

# 2. Define the exact list of columns you want to keep
target_cols = [
    'OBJECTID', 'ID', 'TYPE', 'STATUS', 'SOURCEDATE', 
    'VAL_METHOD', 'VAL_DATE', 'OWNER', 'VOLTAGE', 'VOLT_CLASS',
    'INFERRED', 'Shape__Length'
]

# 3. Filter the dataframe to only include these columns
# Using list comprehension to avoid errors if a specific column is missing
data = data[[col for col in target_cols if col in data.columns]]

print(f"Mapping complete. Current columns: {data.columns.tolist()}")

# check dataset format matches existing one
transmission_df = pd.read_parquet(f"{path_configs['path_data_processed']}/transmission-lines/data.parquet")

# check common columns
common_cols = set(data.columns).intersection(set(transmission_df.columns))
print(f"Common columns: {common_cols}")
print(f"Number of common columns: {len(common_cols)/len(transmission_df.columns)*100:.2f}%")

#diff in columns - they need to be the same for a fair comparison of results. If there are differences, we need to check if they are important and if we can keep them or not.
diff_cols = set(transmission_df.columns).difference(set(data.columns))
print(f"Columns in existing dataset but not in new dataset: {diff_cols}")

diff_cols = set(data.columns).difference(set(transmission_df.columns))
print(f"Columns in new dataset but not in existing dataset: {diff_cols}")
...
# Target engineering
data.loc[data.VOLTAGE == 161, target_name] = '100-161'
data.loc[data.VOLTAGE == 169, target_name] = '100-161'
data = data[data[target_name]!= 'DC'].reset_index(drop=True)
data.loc[~data[target_name].isin(['100-161', 'UNDER 100']), target_name] = '220 AND ABOVE'
...
drop_col = []
# Leakage
drop_col.append('VOLTAGE')
# Highly related
# drop_col.append('SOURCE')
# drop_col.append('SUB_1')
# drop_col.append('SUB_2')
...
#check that after preprocessing full dataset has the exact same columns as the existing one 
if set(data.columns) != set(transmission_df.columns):
    print("Columns in new dataset but not in existing dataset:")
    print(set(data.columns) - set(transmission_df.columns))
    print("Columns in existing dataset but not in new dataset:")
    print(set(transmission_df.columns) - set(data.columns))

## Save data and config
## Change appropriately the save_folder
data_filename = 'transmission-lines-FULL'
