"""Preprocess euro-med-agency_medicines.
The task is to predict whether a medicine is authorized for marketing. (classification)
refer to https://www.ema.europa.eu/en/medicines/download-medicine-data
"""
...
filename = 'european-medicine-agency/medicines.csv' # Change the filename accordingly
...
# Target engineering
data.loc[data[target_name] != 'Authorised', target_name] = 'Not Authorised'
data
...
drop_col = []
leakage_words = ['Withdrawal', 'date', 'Refusal']
for word in leakage_words:
    drop_col += [col for col in data.columns if word in col]
drop_col.append('Revision number')
drop_col.append('Latest procedure affecting product information')
