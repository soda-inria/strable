"""Preprocess michelin_restaurant-ratings."""
...
filename = 'carte/michelin_restaurant-ratings.csv' # Change the filename accordingly
...
for col in ['address', 'review', 'award']:
    loc = data.columns.get_loc(col)
    temp = data[col].copy()
    temp = pd.Series(temp.to_list())
    temp[temp.isnull()] = '{}'
    temp = temp.apply(literal_eval)
    temp = pd.DataFrame(temp)
    temp.columns = ['temp']
    clean_dict_type_col(temp, 'temp', clean_type='unpack')
    drop_col = [col for col in temp if '@' in col]
    drop_col += [col for col in temp if col in data.columns]
    temp.drop(columns=drop_col, inplace=True)
    for col_name in temp.columns:
        data.insert(loc, column=col_name, value=temp[col_name])
        loc += 1
    data.drop(columns=col, inplace=True)
drop_col = [col for col in data if '@' in col]
...
drop_col = []
drop_col += [col for col in data if '@' in col]
drop_col.append('image')
drop_col.append('hasMap')
drop_col.append('url')
drop_col.append('starRating')
drop_col.append('dateAwarded')
data.drop(columns=drop_col, inplace=True)
...
data_filename = 'michelin-ratings'
