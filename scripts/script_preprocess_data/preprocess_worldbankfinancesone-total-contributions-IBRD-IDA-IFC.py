"""Preprocess worldbankfinancesone-total-contributions-IBRD-IDA-IFC.
The task is to predict Total Contribution (USD) (regression)
"""
...
filename = 'worldbankfinancesone/total_contributions_to_ibrd_ida_ifc_trust_funds_summary_based_on_fy_of_agreement_12-17-2025.csv'
...
## Set metadata
target_name = 'Total Contribution (USD)'
task = 'regression'
task_type = 'wide'
...
if task == 'regression':
    ...
    if abs(skewness) > 1:
        check_y = data[target_name].copy()
        check_y = np.sign(check_y) * np.log(np.abs(check_y))
        skewness = stats.skew(check_y)
        kurtosis = stats.kurtosis(check_y)
...
data[target_name] = np.sign(data[target_name]) * np.log(np.abs(data[target_name])) # applied
...
# Leakage
# Highly related
drop_col.append('Contributions Paid In (USD)')
...
data_filename = 'total-contributions-ibrd-ida-ifc'
