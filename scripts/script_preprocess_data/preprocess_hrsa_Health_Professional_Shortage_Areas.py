...
# files = ["BCD_HPSA_FCT_DET_DH.xlsx", "BCD_HPSA_FCT_DET_MH.xlsx", "BCD_HPSA_FCT_DET_PC.xlsx"]
# data_path = f"{path_configs['base_path']}/data/data_raw/health_resources_and_service_administration/"
# data = pd.DataFrame()
# for file in files:
#     full_path = os.path.join(data_path, file)
#     xls = pd.ExcelFile(full_path)
#     df = pd.read_excel(full_path, sheet_name='SHORTAGE_AREA_HP_DETAIL_VX')
#     print(f"File: {file}")
#     print(f"Sheet names: {xls.sheet_names}")
#     print("-" * 50)
#     # check if 'HPSA Score' column exists:
#     if 'HPSA Score' in df.columns:
#         print(f"'HPSA Score' column found in file: {file}")
#     else:
#         print(f"'HPSA Score' column NOT found in file: {file}")
#     print("\n")
#     #concatenate all dataframes
#     data = pd.concat([data, df], ignore_index=True)
filename = 'health_resources_and_service_administration/hrsa-health-professional-shortage-areas.csv'
