"""Preprocess workforce demographics. - wide
The task is to predict Female % (regression).
"""
...
filename = 'health_resources_and_service_administration/AHRF_Demographics_Dashboard_Download_2016-2020.xlsx'
data_path = f"{path_configs['base_path']}/data/data_raw/{filename}"
data = pd.read_excel(data_path, sheet_name='AHRFD_DASH_RPT_VX')
...
drop_col += [
    ...
    'Other or Multiple Races* (N)',
    'Less than 30 Years Old (N)',
    '30 to 39 Years Old (N)',
    '40 to 49 Years Old (N)',
    '50 to 59 Years Old (N)',
    '60 or Older (N)',
    'Female (N)',
    'Male (N)',
    'Male (%)',
]
drop_col.append('Missing or Suppressed Gender Data Comment')
...
data_filename = 'workforce-demographics-wide'
