...
filename = 'fda/drug-drugsfda.json' # Change the filename accordingly
...
data = clean_dict_type_col(data, 'submissions', clean_type='to_string')
data = clean_list_type_col(data, 'submissions')
...
strength_list = []
extract_col = ['reference_drug', 'brand_name', 'reference_standard', 'dosage_form', 'te_code', 'marketing_status']
marketing_status = []
for idx in range(len(temp)):
    strength_drug = []
    if str(temp.iloc[idx]) == 'nan': 
        marketing_status.append({}) 
        strength_drug.append('nan')
    else:
        marketing_ = {col: temp.iloc[idx][0][col] for col in extract_col if col in temp.iloc[idx][0].keys()}
        marketing_status.append(marketing_)
        for x in temp.iloc[idx]:
            if 'strength' not in x['active_ingredients'][0]:
                pass
            else:
                strength_drug.append(x['active_ingredients'][0]['strength'])
    if len(strength_drug) == 0:
        strength_list.append('nan')
    else:
        strength_list.append((', ').join(strength_drug))
strengths = pd.DataFrame(strength_list)
strengths.loc[strengths[0]=='nan', 0] = np.nan
strengths.columns = ['strengths']
marketing_status = pd.DataFrame(marketing_status)
marketing_status.insert(1, column='strengths', value=strengths['strengths'])
data.drop(columns = 'products', inplace=True)
data = pd.concat([data, marketing_status], axis=1)

# Extra steps on cleaning list-type columns
...
# Target engineering
data = data[data[target_name].isin(['Discontinued', 'Prescription'])]
data.reset_index(drop=True, inplace=True)
...
drop_col = []
drop_col.append('openfda') # leakage
drop_col.append('te_code') # leakage
drop_col.append('reference_standard') # high cramer's-v / # leakage
