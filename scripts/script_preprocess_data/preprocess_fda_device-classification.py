"""Preprocess device-classification."""
...
filename = 'fda/device-classification.json' # Change the filename accordingly
...
data = clean_dict_type_col(data, col='openfda', clean_type='to_string')
...
data = data[data[target_name].isin(['1', '2', '3'])]
data.reset_index(drop=True, inplace=True)
...
drop_col = []
drop_col.append('submission_type_id') # high cramer's-v
drop_col.append('medical_specialty') # high cramer's-v
drop_col.append('medical_specialty_description') # high cramer's-v
drop_col.append('openfda') # leakage
drop_col.append('regulation_number') # leakage
...
data_filename = 'device-classification'
