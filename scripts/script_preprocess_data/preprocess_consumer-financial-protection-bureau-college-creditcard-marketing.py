"""Preprocess Consumer-Financial-Protection-Bureau-college-creditcard-marketing
The task is to predict PAYMENTS_BY_ISSUER (regression)
"""
...
filename = 'Consumer Financial Protection Bureau/cfpb_college-credit-card-data-release_2009-2024.csv'
data_path = f'{path_configs['base_path']}/data/data_raw/{filename}'
data = pd.read_csv(data_path, encoding='unicode_escape', low_memory=False)
...
## Set metadata
target_name = 'PAYMENTS_BY_ISSUER'
task = 'regression'
task_type = 'wide'
...
data = data[~data[target_name].str.contains(' .')]
data.reset_index(drop=True, inplace=True)
if task == 'regression':
...
drop_col = []
drop_col.append('Unnamed: 246') # The column not mentioned in the explanation
...
data_filename = 'college-creditcard-marketing'
