"""Preprocess World Resource Institute - Local Government Renewable Action.
The task is to predict the size of purchase. (regression)
refer to 
"""
...
filename = 'world_resource_institute/local_government_renewables_action_tracker_dec_2022.xlsx' # Change the filename accordingly
data_path = f'{path_configs['base_path']}/data/data_raw/{filename}'
data = pd.read_excel(data_path, sheet_name='Transaction Data')
...
data['Announcement Year (or Operations Year if Announcement Year unavailable)'] = data['Announcement Year (or Operations Year if Announcement Year unavailable)'].astype(str)
data["Announcement Date"] = data["Announcement Date"].astype(str)
data['Estimated Annual Generation (MWh)'] = data['Estimated Annual Generation (MWh)'].astype(str)
data['Contract Length (Year)'] = data['Contract Length (Year)'].astype(str)
...
## Set metadata
target_name = 'Size of Purchase (MW)'
task = 'regression'
task_type = 'wide'
...
data_filename = 'local-government-renewable-action'
